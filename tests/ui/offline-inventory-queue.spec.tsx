...
describe('offline-inventory-queue (walk-in cooler resilience)', () => {
  it('does not send another account\'s pending inventory at the same venue', async () => {
    await enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId: 'i1', movementType: 'received', quantity: 2 });
    const recordMovement = vi.fn().mockResolvedValue({});
    await syncOfflineInventoryQueue({ ownerId: 'owner-2', venueId: 'v1', recordMovement });
    expect(recordMovement).not.toHaveBeenCalled();
    expect(await getOfflineQueue('v1')).toHaveLength(1);
    await syncOfflineInventoryQueue({ ownerId: 'owner-1', venueId: 'v1', recordMovement });
  ...
  it('preserves entries enqueued while a sync request is pending', async () => {
    await enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId: 'first', movementType: 'count', quantity: 1 });
    let release!: () => void;
    let started!: () => void;
    const ready = new Promise<void>((resolve) => { started = resolve; });
    const recordMovement = vi.fn(() => { started(); return new Promise<void>((resolve) => { release = resolve; }); });
    const syncing = syncOfflineInventoryQueue({ ownerId: 'owner-1', venueId: 'v1', recordMovement });
    await ready;
    const second = await enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId: 'second', movementType: 'count', quantity: 2 });
    release();
    await syncing;
  ...
  it('serializes concurrent enqueues', async () => {
    await Promise.all(['first', 'second'].map((itemId) => enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId, movementType: 'count', quantity: 1 })));
  ...
  it('does not automatically retry permanent failures, but allows explicit retry', async () => {
    await enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId: 'i1', movementType: 'count', quantity: 1 });
    const recordMovement = vi.fn().mockRejectedValueOnce(Object.assign(new Error('Forbidden'), { status: 403 })).mockResolvedValue({});
    await syncOfflineInventoryQueue({ ownerId: 'owner-1', venueId: 'v1', recordMovement });
    await syncOfflineInventoryQueue({ ownerId: 'owner-1', venueId: 'v1', recordMovement, automatic: true });
  ...
  it('coalesces overlapping sync requests in one runtime', async () => {
    await enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId: 'i1', movementType: 'count', quantity: 1 });
    const recordMovement = vi.fn().mockResolvedValue({});
    await Promise.all([syncOfflineInventoryQueue({ ownerId: 'owner-1', venueId: 'v1', recordMovement }), syncOfflineInventoryQueue({ ownerId: 'owner-1', venueId: 'v1', recordMovement })]);
    expect(recordMovement).toHaveBeenCalledTimes(1);
  });

  it('reuses the operation ID after a lost response and holds later counts for that item', async () => {
    const first = await enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId: 'same', movementType: 'received', quantity: 5 });
    await enqueueOfflineMovement({ ownerId: 'owner-1', venueId: 'v1', itemId: 'same', movementType: 'count', quantity: 8 });
  ...
  it('enqueues a bottle count when in walk-in cooler with zero reception', async () => {
    const item = await enqueueOfflineMovement({ ownerId: 'owner-1',
      venueId: 'v1',
      itemId: 'vodka-1',
      itemName: 'Well Vodka',
      movementType: 'count',
  ...
  it('removes a specific queued movement by ID', async () => {
    const item1 = await enqueueOfflineMovement({ ownerId: 'owner-1',
      venueId: 'v1',
      itemId: 'rum-1',
      movementType: 'count',
      quantity: 5,
    });
    const item2 = await enqueueOfflineMovement({ ownerId: 'owner-1',
      venueId: 'v1',
      itemId: 'rum-2',
      movementType: 'count',
      quantity: 10,
  ...
  it('synchronizes queued movements sequentially upon reconnecting', async () => {
    await enqueueOfflineMovement({ ownerId: 'owner-1',
      venueId: 'v1',
      itemId: 'whiskey-1',
      movementType: 'count',
      quantity: 12,
    ...
    const result = await syncOfflineInventoryQueue({ ownerId: 'owner-1',
      venueId: 'v1',
