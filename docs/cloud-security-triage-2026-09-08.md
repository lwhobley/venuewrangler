...
| --- | --- | --- |
| P1 | Enabled `default-allow-ssh` and `default-allow-rdp` allow TCP 22/3389 from `0.0.0.0/0`, with no target restriction | Matches the likely two critical findings. No Compute instances were returned at inspection time, so live host exposure is not established. Confirm any admin-access dependency, then disable these two rules reversibly; preserve an approved administration path if needed. Requires explicit cloud-change approval. |
| P1 | Both `cloud-run-source-deploy` and `stadium-wrangler` Docker repositories report `SCANNING_DISABLED`; Container Scanning API is not enabled | npm audit does not cover operating-system packages in container images. Enable image vulnerability scanning after approving ongoing scan charges; review findings on actually deployed digests. Do not auto-block production on untriaged historical images. |
